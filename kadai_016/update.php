...
if (isset($_POST['submit'])) {
    try {
        ...
        // bindValue()メソッドで、実際の値をプレースホルダにバインド。
        ...
        // 書籍一覧ページにリダイレクトさせる、messageパラメータも渡す。
...
if (isset($_GET['id'])) {
    try{
        ...
        // idパラメータの値と同じidのデータが存在しなければエラーメッセージを表示して処理終了。
        if ($book === FALSE) {
            exit('idパラメータの値が不正です。');
        }

        // genresテーブルからgenre_codeカラムのデータを取得するためのSQL文を変数$sql_select_genre_codeに代入。
            ...
            <form action="update.php?id=<?=$_GET['id'] ?>" method="post" class="registration-form">
                <div>
                    ...
                    <select id="genre_code" name="genre_code" required>
                        ...
                        <?php
                        // 配列の中身を順に取り出し、セレクトボックスの選択肢として出力
